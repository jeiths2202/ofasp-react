...
class CmdRunner:
    ...
    def _execute_call_command(self, command_req: CommandRequest, start_time: float) -> ExecutionResult:
        """Execute CALL command with enhanced process management"""
        try:
            # For initial implementation, execute directly within cmdRunner
            # TODO: Implement proper fork-based execution in Phase 2
            logger.info(f"Executing CALL command directly: {command_req.command}")
            ...
            try:
                ...
                logger.info(f"CALL command completed: success={success}")
                ...
                logger.error(f"CALL command execution error: {e}")
                return ExecutionResult(
                    success=False,
                    return_code=999,
                    output=stdout_capture.getvalue(),
            ...
            logger.error(f"CALL command execution failed: {e}")
            return ExecutionResult(
                success=False,
                return_code=999,
                output="",
    ...
    def _setup_child_dataset_handlers(self, comm: ParentChildCommunicator):
        ...
        # This would require modifying OVRF/DLTOVR functions to use communication
        # For now, we'll use environment variables to indicate we're in a child process
