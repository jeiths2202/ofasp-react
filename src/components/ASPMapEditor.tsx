...
const ASPMapEditor: React.FC<ASPMapEditorProps> = ({ isDarkMode }) => {
  ...
  const reloadCatalogData = async () => {
    try {
      setStatusMessage(t('mapEditor.reloadingCatalog'));
      
      // Reload all catalog data in parallel
      await Promise.all([
        loadSmedFiles(),
        loadCatalogMaps(),
        loadVolumes()
      ...
      setStatusMessage(t('mapEditor.catalogReloaded'));
      ...
      console.error('Error reloading catalog data:', error);
      setStatusMessage(t('mapEditor.catalogReloadError'));
  ...
  const parseSmedContent = (content: string) => {
    ...
    lines.forEach(line => {
      if (line.startsWith('ITEM')) {
        ...
        if (match) {
          ...
          // Correct type mapping based on SMED file format
          // T = Text, I = Input, O = Output, B = Button
          let fieldType: 'text' | 'input' | 'output' | 'button' = 'text';
          switch(type) {
            case 'I':
            case 'N': // Legacy support
              fieldType = 'input';
              break;
            case 'O':
    ...
    console.log(`Parsed ${counter} fields from SMED content`);
  };
  ...
  const loadCatalogMap = async (map: any) => {
    try {
      // Use the correct API endpoint for volume/library/mapname structure
      const response = await fetch(`http://localhost:8000/api/smed/content/${map.volume}/${map.library}/${map.mapfile}`);
      if (response.ok) {
        const content = await response.text();
        parseSmedContent(content);
        ...
        console.error('Failed to load catalog map:', errorText);
        setStatusMessage(t('mapEditor.fileLoadError', { filename: map.name }));
      }
    } catch (error) {
      console.error('Error loading catalog map:', error);
      setStatusMessage(t('mapEditor.fileLoadException', { error: String(error) }));
    }
  };
        ...
        <div className="bg-gray-800 rounded-lg p-2 mb-2 flex-shrink-0">
          <div className="flex flex-wrap gap-2 items-center text-sm">
            ...
            <div className="flex flex-wrap gap-1">
              ...
              <button
                onClick={reloadCatalogData}
                className="px-2 py-1 text-xs bg-yellow-600 hover:bg-yellow-700 rounded flex items-center gap-1 transition-colors"
                title="Reload catalog data"
              >
                <ArrowPathIcon className="w-3 h-3" />
                {t('mapEditor.reloadCatalog')}
              </button>
