...
const app = express();
const PORT = process.env.FILE_API_PORT || 3006;
...
app.get('/api/datasets/:volume/:library/:dataset/data', async (req, res) => {
  ...
  try {
    ...
    try {
      const catalogPath = path.join('/home/aspuser/app/asp-manager/public/config/catalog.json');
    ...
    try {
      ...
      if (rectype === 'FB') {
        // Fixed Block: Handle as binary data with fixed record length
        // No newlines expected - data is continuous fixed-length records
        if (datasetEncoding === 'shift_jis' || datasetEncoding === 'sjis') {
          // Convert SJIS encoded binary data to Unicode
