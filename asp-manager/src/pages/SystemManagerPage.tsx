...
const SystemManagerPage: React.FC = () => {
  ...
  const fetchJobs = async () => {
    try {
      const response = await fetch('http://localhost:3006/api/jobs');
      if (response.ok) {
        const data = await response.json();
        setJobs(data.jobs || []);
      }
  ...
  const submitJob = async (program: string, library: string, volume: string) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3006/api/jobs/submit', {
        method: 'POST',
        headers: {
  ...
  const handleJobAction = async (jobId: string, action: string) => {
    try {
      const response = await fetch(`http://localhost:3006/api/jobs/${jobId}/${action}`, {
        method: 'POST'
      });
  ...
  const handleDeleteJob = async (jobId: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:3006/api/jobs/${jobId}`, {
        method: 'DELETE'
      });
  ...
  const fetchJobLog = async (job: Job) => {
    try {
      const response = await fetch(`http://localhost:3006/api/jobs/${job.id}/log`);
      if (response.ok) {
  ...
  const handleCLDoubleClick = async (resource: {resource: CatalogResource, name: string, library: string, volume: string}) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3006/api/cl/${resource.volume}/${resource.library}/${filename}`);
  ...
  const executeCL = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3006/api/cl/execute', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const viewDataset = async (dataset: DatasetInfo) => {
    ...
    try {
      const response = await fetch(`http://localhost:3006/api/datasets/${dataset.volume}/${dataset.library}/${dataset.name}/data`);
      if (response.ok) {
  ...
  const saveDatasetChanges = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3006/api/datasets/${editingDataset.volume}/${editingDataset.library}/${editingDataset.name}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const deleteDataset = async (dataset: DatasetInfo) => {
    if (window.confirm(`データセット "${dataset.name}" を削除しますか？\n\nライブラリ: ${dataset.library}\nボリューム: ${dataset.volume}\n\nこの操作は取り消せません。`)) {
      try {
        const response = await fetch(`http://localhost:3006/api/datasets/${dataset.volume}/${dataset.library}/${dataset.name}`, {
          method: 'DELETE'
        });
        if (response.ok) {
  ...
  const formatDatasetContent = (data: string, rectype: string, reclen: number, page: number = 1, pageSize: number = 50, encoding: string = 'utf8') => {
    ...
    switch (rectype?.toUpperCase()) {
      case 'FB': {
        // Fixed Block - 固定長レコード (바이트 단위 처리)
        while (byteOffset < getByteLength(data, encoding)) {
          const record = sliceByBytes(data, byteOffset, validReclen, encoding);
