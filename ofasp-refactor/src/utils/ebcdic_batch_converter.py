...
class EbcdicBatchConverter:
    ...
    def __init__(self, codepage_base_path: str = None):
        ...
        # SOSIコード定数
        self.SOSI_SO = 0x0E  # Shift Out
        self.SOSI_SI = 0x0F  # Shift In
    ...
    def EBCDIC_TO_ASCII(self, 
        ...
        while i < len(bytes_data):
                ...
                if is_double_byte_mode:
                    if i + 1 < len(bytes_data):
                        ...
                        if double_byte_value in double_byte_table:
                            ...
                            else:  # ダブルバイト結果 - Shift-JIS処理
                                if encoding == 'JP' and mapped_value >= 0x8140:
                                    try:
                                        # Shift-JISバイトに変換後文字にデコード
                                        ...
                                        print(f"  -> Shift-JIS文字: 0x{mapped_value:04X} -> '{sjis_char}'")
                                    except UnicodeDecodeError:
                                        result.append('?')  # 変換失敗時クエスチョンマーク
                                        print(f"  -> Shift-JISデコードエラー: 0x{mapped_value:04X} -> '?'")
                                else:
                                    result.append(' ')  # 安全な代替
            ...
            result.append(chr(ascii_value))
            print(f"[{i:02d}] シングルバイト: 0x{byte:02X} -> 0x{ascii_value:02X} ('{chr(ascii_value)}')")
