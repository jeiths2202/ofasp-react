...
def OVRF(command_line: str) -> bool:
    ...
    try:
        ...
        if _is_child_process():
            print(f"[OVRF] Running in child process, requesting from parent")
            success, message = _request_dataset_from_parent(logical_name, physical_file)
            if success:
                print(f"[OVRF] Parent allocated dataset successfully")
            else:
                print(f"[ERROR] Parent allocation failed: {message}")
            return success
