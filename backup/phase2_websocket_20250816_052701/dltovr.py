...
def DLTOVR(command_line: str) -> bool:
    ...
    try:
        ...
        if _is_child_process():
            print(f"[DLTOVR] Running in child process, requesting from parent")
            success, message = _request_dataset_dealloc_from_parent(logical_name)
            if success:
                print(f"[DLTOVR] Parent deallocated dataset successfully")
            else:
                print(f"[ERROR] Parent deallocation failed: {message}")
            return success
