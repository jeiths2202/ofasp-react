    ...
    def load_smed_map(self, map_file, output_data):
        ...
        try:
            ...
            if os.path.exists(map_file):
                # Console terminal display - no conversion needed (preserve original encoding)
                self.map_data = parse_smed_file(map_file, destination='terminal')
            else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='SMED Map Display Engine')
    parser.add_argument('map_name', help='SMED map name')
    parser.add_argument('disk', nargs='?', default='DISK01', help='Disk name (default: DISK01)')
    parser.add_argument('library', nargs='?', default='TESTLIB', help='Library name (default: TESTLIB)')
    
    args = parser.parse_args()
    
    # Construct file path
    map_file_path = f"/home/aspuser/app/volume/{args.disk}/{args.library}/{args.map_name}"
    
    # Check if file exists
    if not os.path.exists(map_file_path):
        print(f"Error: SMED map file not found: {map_file_path}")
        print("Available files:")
        try:
            volume_path = f"/home/aspuser/app/volume/{args.disk}/{args.library}"
            if os.path.exists(volume_path):
                files = [f for f in os.listdir(volume_path) if os.path.isfile(os.path.join(volume_path, f))]
                for f in sorted(files):
                    print(f"  {f}")
            else:
                print(f"  Directory not found: {volume_path}")
        except Exception as e:
            print(f"  Error listing files: {e}")
        sys.exit(1)
    
    # Default output data for testing
    test_output_data = {
        "fields": {},
        "messages": [
            f"Loading SMED map: {args.map_name}",
            f"Disk: {args.disk}, Library: {args.library}",
            "Press F1 for help, F3 to exit"
        ]
    }
    
    print(f"Loading SMED map: {map_file_path}")
    success, values, message = display_smed_map_terminal(
        map_file_path, 
        test_output_data, 
        f"SMED-{args.map_name}"
    )
